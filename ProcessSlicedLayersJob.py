    ...
    def run(self):
        ...
        for layer in self._layers:
            ...
            for p in range(layer.repeatedMessageCount("path_segment")):
                ...
                _gs = Application.getInstance().getGlobalContainerStack()
                if _gs and _gs.getProperty("belt_raft", "value"):
                    raft_thickness = _gs.getProperty("belt_raft_thickness", "value") or 0.8
                ...
                if Application.getInstance().getGlobalContainerStack() and Application.getInstance().getGlobalContainerStack().getProperty("belt_wall_enabled", "value"):
                    belt_wall_feedrate = Application.getInstance().getGlobalContainerStack().getProperty("belt_wall_speed", "value") or 600.0
                    belt_wall_indices = []
        ...
        if transform and transform != Matrix():
            ...
            if global_stack and global_stack.getProperty("belt_raft", "value"):
                front_offset = front_offset - (global_stack.getProperty("belt_raft_margin", "value") or 0.0) \
                                            - (global_stack.getProperty("belt_raft_thickness", "value") or 0.8)
